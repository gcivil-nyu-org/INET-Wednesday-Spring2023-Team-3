...
class QuestionsModelTest(TestCase):
    def test_question_creation_success(self):
        title, description, companies, categories, difficulty, positions, type = (
            "t1",
            "desc1",
            "c1,c2",
            "cat1,cat2",
        ...
        self.assertEqual(
            question.__str__(), f"{question.q_id} : {type} , {title} , {difficulty}"
        )
...
class QuestionViewTest(TestCase):
    ...
    def test_post_question_success(self):
        ...
        response = self.client.post(
            reverse("post_question"), data=req_body, content_type="application/json"
        )
        ...
        self.assertEqual(
            1, Difficulty.objects.filter(text=req_body["difficulty"]).count()
        )
    ...
    def test_post_question_failure_missingparam(self):
        response = self.client.post(
            reverse("post_question"), data={}, content_type="application/json"
        )
        self.assertIsNotNone(response)
        response_dict = json.loads(response.content)
        self.assertIsNotNone(response_dict)
    ...
    def assert_success_num_questions(self, param_vals, status, error_msg, qnum):
        ...
        self.assertEqual(
            len(param_vals[3]), len(sample_question["companies"].split(","))
        )
        self.assertEqual(
            len(param_vals[4]), len(sample_question["positions"].split(","))
        )
    ...
    def test_get_question_success_plain(self):
        ...
        self.assert_success_num_questions(
            param_vals=param_vals, status=200, error_msg="", qnum=1
        )
    ...
    def test_get_question_params(self):
        ...
        urls_expected_responses = {
            ...
            "?cur_page=string&single_page_count=string": [
                0,
                400,
                "Error: Pagination params not valid!",
            ],
        }
        ...
        for key, val in urls_expected_responses.items():
            ...
            self.assert_success_num_questions(
                param_vals=param_vals, status=val[1], error_msg=val[2], qnum=val[0]
            )
